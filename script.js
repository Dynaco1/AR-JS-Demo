...
const minDistanceFromLatLong = 10; // Minimum distance in meters from the target lat/long
const fixedDistanceFromCamera = 3; // Distance in meters to keep the object in front of the camera
const fixedScale = 0.09;
...
window.onload = () => {
    ...
    el.addEventListener("gps-camera-update-position", e => {
        ...
        // Check the distance for each entity and adjust its position relative to the camera
        entities.forEach((entity, index) => {
            checkDistanceAndPosition(e, entity, locations[index].lat, locations[index].long);
        });
    });
};

// Function to check distance and update position of entity relative to the camera
function checkDistanceAndPosition(e, entity, destinationLat, destinationLong) {
    const currentLat = e.detail.position.latitude;
    const currentLong = e.detail.position.longitude;
...
// Function to update entity's position relative to the camera
function updatePositionRelativeToCamera(entity, distance) {
    const camera = document.querySelector("[gps-new-camera]");
    const cameraPosition = camera.object3D.position;

    // Set entity position in front of the camera at the fixed distance
    const cameraDirection = new THREE.Vector3();
    camera.object3D.getWorldDirection(cameraDirection);
    const entityPosition = cameraPosition.clone().add(cameraDirection.multiplyScalar(distance));

    entity.setAttribute('position', {
        x: entityPosition.x,
        y: cameraPosition.y, // Keep the entity at the same height as the camera
        z: entityPosition.z
    });
}
